...
pub fn rml(input: TokenStream) -> TokenStream {
    ...
    let res = syn::parse::Parser::parse(
        |transformed_input: ParseStream| {
    ...
    let generated = parsed_node.generate_with_components_and_counter(
        &components,
        &mut 0,
        &properties_mapping,
        "",
    ...
    let generated_initializer = inject_engine_text_based(
    ...
    let generated_initializer = generated_initializer
    ...
    let generated_functions =
        inject_engine_text_based(&generated_functions, "engine", true, true, &functions_name);
    let generated_functions = generated_functions
        .parse::<proc_macro2::TokenStream>()
        .unwrap();

    let generated_node = generated_node.to_string();
    let generated_node =
        inject_engine_text_based(&generated_node, "engine", false, false, &functions_name);
