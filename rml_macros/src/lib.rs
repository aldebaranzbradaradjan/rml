...
pub fn rml(input: TokenStream) -> TokenStream {
    ...
    println!("Generated functions: {}", generated_functions);
    ...
    let result = quote! {
        {
            ...
            // generate the nodes
            #generated_node; // tree computed here
            
            // add code for founded functions 
...
impl RmlNode {
    fn generate(&self) -> (String, proc_macro2::TokenStream, proc_macro2::TokenStream) {
        ...
        let functions_of_childs: Vec<proc_macro2::TokenStream> = child_results
            .iter()
            .map(|(_, _, functions)| functions.clone())
            .collect();
        ...
        // let functions_code: Vec<proc_macro2::TokenStream> = self
        //     .functions
        //     .iter()
        //     .map(|f| {
        //         let f_name = f.sig.ident.clone();
        ...
        println!("just before end {}", functions_code);

        (id, node_code, functions_code)
    }
}
