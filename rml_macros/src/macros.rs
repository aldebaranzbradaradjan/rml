...
impl RmlNode {
    ...
    pub fn generate_with_components_and_counter(
        ...
        node_hierarchy: &Vec<(String, Vec<String>)>,
        parent_id: &str,
        ...
        if node_type != ItemTypeEnum::Repeater {
            ...
            let r_child_nodes_id: Vec<String> = child_results
                ...
                .collect();
            if r_child_nodes_id.len() != 1 {
                panic!("Repeater node must have only one child");
            }
            ...
            let block_string = transform_dollar_this_parent_syntax(&r_child_node_id, &id, &block_string, properties_mapping, node_hierarchy);
            ...
            // a last thing, we need to diversify all the ids in the child code for each call of the repeater create function
            let mut replacement_map= Vec::new();
            collect_children(node_hierarchy, &id, &mut replacement_map);

            //let replacement_map = generate_id_replacement_map(&r_child_code.to_string(), node_hierarchy);
                    ...
                    let value = match v {
                        Value::Block(oblock) => {
                            ...
                            if !parent_is_repeater {                   
                                ...
                                let block_string = transform_dollar_this_parent_syntax(&id, &parent_id, &block_string, properties_mapping, node_hierarchy);
    ...
    fn generate_custom_component_with_counter(
        ...
        let file_content = transform_dollar_syntax(&file_content, properties_mapping, node_hierarchy);
    ...
    pub fn pre_generate_with_components_and_counter(&mut self, components: &HashMap<String, ComponentDefinition>, id_counter: &mut u32) ->
        (HashMap<String, AbstractValue>, Vec<(String, Vec<String>)>)
    {
        ...
        // add it to self.properties if not present
        if !self.properties.iter().any(|(_t, k, _v)| k.to_string() == "id") {
            self.properties.push((PropertyType::String, PropertyKey::Simple(Ident::new("id", Span::call_site())), Value::Lit(Lit::Str(syn::LitStr::new(&id, Span::call_site())))));
        }

        //println!("Pre-generating node of type: {} with id: {}", self._ident, id);

        let mut node_hierachy: Vec<(String, Vec<String>)> = Vec::new();
        let mut childs_hierachy: Vec<String> = Vec::new();
        ...
        for i in 0..self.children.len() {
            ...
            let child_id: String = self.children[i]
                .properties
                .iter()
                .find_map(|(_t, k, v)| {
                    if k.to_string() == "id" { 
                ...
                // add it to self.properties if not present
            if !self.children[i].properties.iter().any(|(_t, k, _v)| k.to_string() == "id") {
                self.children[i].properties.push((PropertyType::String, PropertyKey::Simple(Ident::new("id", Span::call_site())), Value::Lit(Lit::Str(syn::LitStr::new(&child_id.clone(), Span::call_site())))));
            }
            childs_hierachy.push(child_id.clone());

            let res = self.children[i].pre_generate_with_components_and_counter(components, id_counter);
            merged_child_results.extend(res.0);
            node_hierachy.extend(res.1);
        }

        node_hierachy.push((id.clone(), childs_hierachy));
